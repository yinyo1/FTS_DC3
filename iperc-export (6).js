...
async function generateExcel(){
  ...
  try{
    ...
    function _fixVerif(txt){
      return _cn(txt||'').replace(/^([Vv]erificar|[Rr]evisar|[Cc]onfirmar|[Aa]segurar)\b/g,
        function(m){return 'Vamos a '+m.toLowerCase();});
    }

    // ── Grupos con personal roles ────────────────────────────────────
    ...
    groups.forEach(function(g){
      ...
      g.risks.forEach(function(rsk,ri3){
        ...
        var sD={sz:10,ha:'left',va:'top',wrap:true,bdr:BT};
        var sN={sz:12,ha:'center',va:'middle',wrap:false,bdr:BT};

        // Col 4: tipo peligro
        sc(ws.getCell(dr,4),rsk.tipo||'—',{ha:'center',va:'middle',sz:10,bdr:BT});
        // Col 5: riesgo + consec merged
        var rDesc=_cn(rsk.riesgo||'—')+(rsk.consec?'\n'+_cn(rsk.consec):'');
        sc(ws.getCell(dr,5),rDesc,sD);
        // Col 6: ¿Quién? = personal roles
        sc(ws.getCell(dr,6),g.personalStr||_cn(rsk.consec||'—'),{sz:10,ha:'left',va:'top',wrap:true,bdr:BT});
        // C E P GR inherente
  ...
  try{
    ...
    function _pdfFullDesc(actName){
      ...
      if(ra.consideraciones){
        var verif=_cleanNomRefs(ra.consideraciones)
          .replace(/^(Verificar|Revisar|Confirmar|Asegurar|Inspeccionar)/gi,function(m){
            return 'Vamos a '+m.toLowerCase();
          });
    ...
    pdfGroups.forEach(function(group, gi){
      ...
      group.risks.forEach(function(r, ri){
        ...
        row.push({content:_rDesc,styles:{fillColor:C_WHITE}});
        ...
        row.push({content:_quien,styles:{fillColor:C_WHITE,fontSize:6}});
        // C E P GR inherente

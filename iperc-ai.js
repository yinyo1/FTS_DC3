...
'══ REGLA 3 · PRIMERA PERSONA OBLIGATORIA ══\n'+
'Escribe TODO como supervisor FTS explicando en voz alta al supervisor EHS del cliente.\n'+
'USA: "Primero vamos a...", "El técnico electromecánico va a instalar...", "Vamos a subir con andamio multidireccional a X metros...", "Una vez que tengamos el LOTO aplicado, procedemos a..."\n'+
...
'  • QUIÉN: rol específico ("Electricista certificado NOM-029", "Armador de andamio NOM-009", "Ayudante general")\n'+
...
'  • "consideraciones": checklist que el supervisor firma ANTES de iniciar la fase (permisos, EPP, NOMs aplicables)\n'+
...
'{"actividades":[{"paso":"1","nombre":"NOMBRE FASE EN MAYÚSCULAS","descripcion":"Descripción ejecutiva de la fase","consideraciones":"Checklist supervisor ANTES de iniciar","nota":"CONDICIÓN CRÍTICA EN MAYÚSCULAS si aplica (zona tráfico, planta activa, LOTO, etc.)","subpasos":[{"paso":"1.1","descripcion":"1.- Vamos a [acción] usando [herramienta/equipo + especificaciones]. [Quién] va a [qué hace]. [Coordinación/condición especial]","personal":"Rol del trabajador (ej: electricista certificado NOM-029, operador montacargas)"}]}]}';
        ...
        if(Array.isArray(acts)){
          ...
          kbActNames.forEach(function(kbName){
            ...
            const already = acts.find(function(a){
              return a.nombre===kbName||_stripSimple(a.nombre)===kbS
                ||(kbS.length>5&&_stripSimple(a.nombre||'').includes(kbS.substring(0,15)))
                ||(kbS.length>5&&kbS.includes(_stripSimple(a.nombre||'').substring(0,15)));
            });
            if(!already){
              ...
              if(/andamio/i.test(kbName)){
                ...
                kbSubs=(kbData.noms||[]).length
                  ? [{paso:'1',descripcion:'Verificar permisos y aplicar '+(kbData.noms.join(', '))+' antes de iniciar.',personal:'Supervisor del contratista'}]
                  : [{paso:'1',descripcion:'Ejecutar según procedimientos del contratista y normativa NOM-STPS aplicable.',personal:'Supervisor del contratista'}];
              }
              ...
              window._rawActividades.push({nombre:kbName,name:kbName,paso:'',descripcion:kbDesc,
                consideraciones:'Verificar permisos vigentes y EPP completo antes de iniciar. Charla de seguridad con todo el equipo.',
          ...
          acts.forEach(function(a){
            ...
            const fullDesc=[
              ...
              Array.isArray(a.subpasos)?a.subpasos.map(function(s){return s.paso+'. '+(s.personal?'['+s.personal+'] ':'')+s.descripcion;}).join('\n'):'',
            ].filter(Boolean).join('\n\n');
